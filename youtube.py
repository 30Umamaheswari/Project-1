...
def calculate_average_duration(rows):
    ...
    for channel_title in channel_durations:
        ...
        avg_duration_data.append([channel_title, f"{int(average_duration_seconds // 3600):02}:{int((average_duration_seconds % 3600) // 60):02}:{int(average_duration_seconds % 60):02}"])
...
def get_video_data(v):
    ...
    for i in range(len(v)):
        ...
        video_data = {
            ...
            "video_caption_status": "Available" if "caption" in video_response["items"][0]["contentDetails"] else "Unavailable",
...
def main():
    ...
    st.set_page_config(page_title="Youtube Data Retrieving and Storing in different DB",
                       layout="wide",
    ...
    with st.sidebar:
        selected = option_menu('Harvesting Menu', ["Retrieve & Store", "Data Analysis"],
                               ...
                               styles={"nav-link": {"font-size": "15px", "text-align": "centre", "margin": "0px",
                                       ...
                                       "nav-link-selected": {"background-color": "#aacb88"}})
    ...
    if selected == "Data Analysis":
        ...
        filter_data = st.selectbox('filters', ['To view the names of all the videos and their corresponding channels.',
        ...
        if filter_data == 'To view the names of all the videos and their corresponding channels.':
            cursor.execute("SELECT video_data.video_name, channel_data.channel_title FROM video_data JOIN channel_data ON video_data.channel_id = channel_data.channel_id")
            df = pd.DataFrame(cursor.fetchall(), columns=cursor.column_names)
            st.table(df)

        elif filter_data == 'To view which channels have the most number of videos, and view how many videos that they have.':
            cursor.execute("SELECT channel_data.channel_title, COUNT(video_data.video_id) AS VideoCount FROM channel_data JOIN video_data ON channel_data.channel_id = video_data.channel_id GROUP BY channel_data.channel_title ORDER BY VideoCount DESC;")
            df = pd.DataFrame(cursor.fetchall(), columns=cursor.column_names)
            st.table(df)

        elif filter_data == 'The top 10 most viewed videos and their respective channels.':
            cursor.execute('SELECT video_data.video_name, channel_data.channel_title, video_data.video_view_count FROM video_data JOIN channel_data ON video_data.channel_id = channel_data.channel_id ORDER BY video_data.video_view_count DESC LIMIT 10')
            df = pd.DataFrame(cursor.fetchall(), columns=cursor.column_names)
            st.table(df)

        elif filter_data == 'Total comments were made on each video, and their corresponding video names.':
            cursor.execute("SELECT video_data.video_name, COUNT(comment_data.comment_id) AS CommentCount FROM video_data LEFT JOIN comment_data ON video_data.video_id = comment_data.video_id GROUP BY video_data.video_name")
            df = pd.DataFrame(cursor.fetchall(), columns=cursor.column_names)
            st.table(df)

        elif filter_data == 'To view which videos have the highest number of likes, and their corresponding channel names.':
            cursor.execute("SELECT video_data.video_name, channel_data.channel_title FROM video_data JOIN channel_data ON video_data.channel_id = channel_data.channel_id ORDER BY video_data.video_likes DESC LIMIT 10")
            df = pd.DataFrame(cursor.fetchall(), columns=cursor.column_names)
            st.table(df)

        elif filter_data == 'The total number of likes and dislikes for each video, and their corresponding video names.':
            cursor.execute("SELECT video_data.video_name, SUM(video_data.video_likes) AS TotalLikes, SUM(video_data.video_dislikes) AS TotalDislikes FROM video_data JOIN channel_data ON video_data.channel_id = channel_data.channel_id GROUP BY video_data.video_name")
            df = pd.DataFrame(cursor.fetchall(), columns=cursor.column_names)
            st.table(df)

        elif filter_data == 'The total number of views for each channel, and their corresponding channel names.':
            cursor.execute("SELECT channel_data.channel_title, SUM(video_data.video_view_count) AS TotalViews FROM channel_data JOIN video_data ON channel_data.channel_id = video_data.channel_id GROUP BY channel_data.channel_title")
            df = pd.DataFrame(cursor.fetchall(), columns=cursor.column_names)
            st.table(df)

        elif filter_data == 'The names of all the channels that have published videos in the year 2022.':
            cursor.execute("SELECT DISTINCT channel_data.channel_title FROM channel_data JOIN video_data ON channel_data.channel_id = video_data.channel_id WHERE video_data.video_published_at LIKE '2022%'")
            df = pd.DataFrame(cursor.fetchall(), columns=cursor.column_names)
            st.table(df)


        elif filter_data == 'The average duration of all videos in each channel, and their corresponding channel names.':
            cursor.execute("SELECT channel_data.channel_title, video_data.video_duration FROM channel_data JOIN video_data ON channel_data.channel_id = video_data.channel_id")
        ...
        elif filter_data == 'To view which videos have the highest number of comments, and their corresponding channel names.':
            cursor.execute("SELECT video_data.video_name, channel_data.channel_title FROM video_data JOIN channel_data ON video_data.channel_id = channel_data.channel_id ORDER BY video_data.video_comments_count DESC LIMIT 10")
            df = pd.DataFrame(cursor.fetchall(), columns=cursor.column_names)
            st.table(df)
...
# [theme]
# base="light"
# backgroundColor="#e8e497"
# secondaryBackgroundColor="#aacb88"
# font="serif"
